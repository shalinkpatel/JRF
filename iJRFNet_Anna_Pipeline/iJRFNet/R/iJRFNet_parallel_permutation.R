...
"iJRF_parallel_permutation" <-
  function(X, W=NULL,ntree=NULL,mtry=NULL,genes.name=NULL,parallel,seed) {
...
# --- MAIN function
"iJRFNet_parallel_permutation" <-
  function(X, W=NULL,ntree=NULL,mtry=NULL,model=NULL,genes.name,ptm.name=NULL,parallel,seed) {
    
     if (is.null(model)) {print("Error: Specify Model")} else { 
      if (is.null(ntree)) ntree=1000
      
      if (model=="iJRF")  out<-iJRF_parallel_permutation(X=X,W=W,ntree=ntree,mtry=mtry,genes.name=genes.name,parallel=parallel,seed=seed)
      if (model=="iRafNet") out<-iRafNet_parallel_permutation(X=X, W=W,ntree=ntree,mtry=mtry,genes.name=genes.name,parallel=parallel,seed=seed)
      if (model=="ptmJRF") out<-ptmJRF_parallel_permutation(X=X,ntree=ntree,mtry=mtry,genes.name=genes.name,ptm.name=ptm.name,parallel=parallel,seed=seed)
      return(out)
    }
  }