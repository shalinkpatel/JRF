...
"ptmJRF_onetarget" <-
  function(x, y=NULL, p,mptm, xtest=NULL, ytest=NULL, ntree,
           ...
           keep.inbag=FALSE, nclasses, numptm, locptm, ptm.num, ...) {
    ...
    replace=T
      ...
      rfout <- .C("ptmJRF_regRF",
                  ...
                  as.integer(numptm), as.integer(locptm), as.integer(ptm.num))[c(16:28, 36:41)]
    ...
    # --- passed ptm.num to .C call and need to change C code to handle multiple PTM objects
  }


# --- MAIN function
"ptmJRF" <- function(X, ntree=NULL, mtry=NULL, genes.name, ptm.name, ptm.num = 1) {
    ...
    # --- standardize data
    for (j in 1:nclasses) {  
      X[[j]] <- t(apply(X[[j]], 1, function(x) { (x - mean(x)) / sd(x) } ))    
      sampsize[j]<-dim(X[[j]])[2] 
    }
    
    
    # --- reorder rows in PTM objects X[[1]] to X[[ptm.num]]
    X.ptm<-X[1:ptm.num]; s=0 ; locptm<-numptm<-matrix(data = rep(0,p*ptm.num), nrow = ptm.num, ncol = p)
    ptm.new<-ptm.name
    for (i in 1:ptm.num){
      for (j in 1:p){ 
        ptm.j<-X[[i]][ptm.name==genes.name[j],]
        n.j<-sum(ptm.name==genes.name[j])
        X.ptm[[i]][seq(s+1,s+n.j),]<-ptm.j
        locptm[i, j]<-(s+1)
        numptm[i, j]<-n.j
    ...
    for (j in 1:ptm.p){
      ...
      for (c in 1:nclasses)  {
        if (c %in% 1:ptm.num) {
          ...
          covar[seq(1,ptm.p-numptm[c, genes.name==ptm.name[j]]),seq(1,sampsize[c])]<-X[[c]][-seq(locptm[c, genes.name==ptm.name[j]],locptm[c, genes.name==ptm.name[j]]+numptm[c, genes.name==ptm.name[j]]-1),]
          n.covar<-ptm.p-numptm[c, genes.name==ptm.name[j]]
          } 
        else {
      ...
      #EDIT ONE_TARGET TO HANDLE MULTNET
      rfout<-ptmJRF_onetarget(x=covar,y=y,p=(p-1),mptm=ptm.p-numptm[genes.name==ptm.name[j]],
                           ...
                           ntree=ntree,numptm=numptm.j,locptm=locptm.j,ptm.num = ptm.num)
      
      imp.rfout<-importance(rfout)
      for (s in 1:nclasses) imp[genes.name!=ptm.name[j],j,s]<-imp.rfout[seq((p-1)*(s-1)+1,(p-1)*(s-1)+p-1)] 
    }
    
    # --- Edit this not quite sure what it is trying to do
    imp.new<-array(0,c(p,p,nclasses))
    for (j in 1:p){
      if (numptm[1,j] <= ptm.num){  
        for (c in 1:nclasses) imp.new[,j,c]<-imp[,locptm[1,j],c]
      } else {  
        for (c in 1:nclasses) imp.new[,j,c]<-apply(imp[,seq(locptm[1,j],locptm[1,j]+numptm[1,j]-1),c], 1, function(x) { mean(x) } )
      }
    }
